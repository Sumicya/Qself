# Qself 分支隐私补充（2026-09-12，2026-09-16 更新本地诊断条目）

本补充说明此分支的实际实现；下方保留上游条款作为来源记录。

- 已移除模块的 AppCenter SDK、统计事件和自动崩溃上传。不收集或向开发者上传模块加载时的 QQ 号。
- 本地诊断默认开启，只在本机保存元数据：功能启动/初始化结论、开关与选择数量、异常类型和栈位、发送路径的拦截与未拦截结论，以及诊断文件的命令行输出；不读取请求正文、聊天正文、登录票据、密码或验证码。
- 两类本地诊断都在“功能开关与错误记录”里控制：功能记录（每进程有数量与长度上限）与诊断文件（256 KiB 上限）；数字标识按同一规则脱敏；都可以随时关闭或清空。复制摘要到剪贴板是用户主动操作，分享前应再次核对内容。
- 其他保留功能（例如群日志、好友记录、打开共同群页面）仍按各自用途处理相应数据，可能使用宿主或外部网络服务。本补充不表示 QQ 或其他模块停止收集数据。
- “观察到调用”不是“服务器已接收”；此功能不能保证避免平台检测、下线或重新登录。
