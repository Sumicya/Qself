...
class UiAgentItem(
    ...
    private fun bindCell(cell: TitleValueCell, position: Int, context: Context) {
        ...
        val description: CharSequence? = agent.summaryProvider?.invoke(agent, context)?.takeIf {
            !redundantSummary(cell.title, it)
        }
        ...
        if (hasError || cell.isUnavailable) {
            // One short line replaces the description instead of sitting on top of it: the row
            // states its condition and stays the height of every other row. The description is
            // still in the row's accessibility text below.
            cell.summary = if (hasError) "出错 · 长按查看详情" else "当前版本不支持"
        }
        // The row shows title + one line; the full description lives in the accessibility text so
        // a clamp on screen never removes information from the screen reader.
        cell.contentDescription = listOfNotNull(
            cell.title.takeIf { it.isNotBlank() },
            description?.toString()?.takeIf { it.isNotBlank() })
            .joinToString("，")
    ...
    companion object {
        /**
         * A description that only restates the title is noise: the row spends a line on it and the
         * user learns nothing. Titles here are short ("隐藏小程序加载广告"), and some catalog
         * entries carry the same sentence with a suffix ("...功能"), so those are dropped.
         *
         * Comparison ignores whitespace and punctuation (the catalog inserts spaces around latin
         * words) and keeps any description that adds more than a short suffix.
         */
        @JvmStatic
        fun redundantSummary(title: String, summary: CharSequence?): Boolean {
            if (summary.isNullOrBlank()) return true
            val plain = { text: String -> text.filterNot { it.isWhitespace() || it in "，。、；：！？·()（）【】《》|/\\-—_" } }
            val a = plain(title)
            val b = plain(summary.toString())
            if (a.isEmpty() || b.isEmpty()) return false
            if (a == b || a.contains(b)) return true
            return b.contains(a) && b.length <= a.length + 4
        }
