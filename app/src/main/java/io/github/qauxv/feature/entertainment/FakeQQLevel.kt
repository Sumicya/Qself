...
package io.github.qauxv.feature.entertainment
...
import io.github.qauxv.util.ExfriendManager.ConfigData
import io.github.qauxv.feature.ui.view.ConfigView
import io.github.qauxv.util.ignoreResult
 ...
 * 
 * 重构说明：
 * - 迁移至新包结构 io.github.qauxv.feature.entertainment
...
@FunctionHookEntry
@UiItemAgentEntry
object FakeQQLevel : CommonSwitchFunctionHook(
    keyName = "Ketal_FakeQQLevel",
    ...
    private fun showDialog(ctx: Context) {
        throwOrTrue {
            ...
            val dialog = MaterialDialog(ctx).show {
                title(text = "自定义 QQ 等级")
                input(hint = "自定义 QQ 等级...", prefill = level, waitForPositiveButton = false) { dialog, text ->
                    val inputField = dialog.getInputField()
                    dialog.setActionButtonEnabled(WhichButton.POSITIVE, try {
                        val level = text.toString().toInt()
                        if (level < 1 || level > 1000) {
                            inputField.error = "无效数值，仅可以输入 1-1000 范围内的数字"
                            false
                        } else {
