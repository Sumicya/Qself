...
public final class TabBarBridge {
    ...
    public static void install(HostApp app, ClassLoader cl) {
        if (sHooked) {
            return;
        }
        int hooked = 0;
    ...
    static void tryHookPager(ViewGroup pager) {
        ...
        try {
            ...
            if (setItemBool != null) {
                ...
                LiquidGlassModule.hookIntercept(setItemSmooth, param -> {
                    ...
                    if (param.thisObject != LiquidGlassInstaller.currentPager()) {
                        return; // not ours: leave the original call alone
                    }
                    ...
                    if (!smooth) {
                        ...
                        // is now true and this before-advice returns without
                        // touching the result: that is what stops it recursing.
                        setItemSmooth.invoke(param.thisObject, args[0], true);
                        param.setResult(null); // swallow the original hard-cut call
                    }
                    // smooth calls fall through and proceed
                });
                sHookedPager = true;
                LiquidGlassModule.log(android.util.Log.INFO,
