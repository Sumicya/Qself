...
final class LiquidGlassPanel extends View {
    ...
    /** Hardware blur radius in dp (upstream LiquidBottomTabs: 8dp). */
    ...
    // Container surface: upstream LiquidBottomTabs containerColor at 40% alpha.
    private static final int WASH_LIGHT = 0x66FAFAFA;
    private static final int WASH_DARK = 0x66121212;
    // Upstream InteractiveHighlight on the container row: an 8% white PLUS
    // wash plus a 15% radial centred on the droplet (radius = minDim * 1.5).
    ...
    private void paintPressHighlight(Canvas canvas, int w, int h) {
        // Verbatim upstream InteractiveHighlight (catalog utils): flat 8%
        // white PLUS wash + 15% white PLUS radial at the droplet centre.
        if (press <= 0.01f) {
            return;
        }
        ...
        float cx = pressX > 0f ? Math.min(Math.max(pressX, 0f), w) : w * 0.5f;
