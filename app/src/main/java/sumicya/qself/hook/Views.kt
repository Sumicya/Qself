...
/**
 * 跟 view 一起生灭的每-view 状态：view 被回收或扔掉时，undo 和钩子都不会留下来。
 * 状态挂在 view 自己的 tag 上 —— 用 WeakHashMap<View, State> 而 State 里再指回 view 的话，
 * 两个都别想被回收。
 */
...
class Decor<S : DecorState> {
    ...
    private companion object {
        // Tag key 必须长得像个资源 id 但又不属于 android（>= 0x02xxxxxx）。0x5E 谁也不占。
...
/**
 * 一个只动界面的开关：藏掉 QQ 自己的某块 UI。判断靠看得见的东西 —— content description、
 * 文字、几何 —— 不靠混淆后的名字，所以 QQ 更新也活得下去。藏过的都记着，关掉就还回去。
 */
abstract class ViewRule(id: String, val inLists: Boolean = false) : Switch(id) {
    internal val hidden = WeakHashMap<View, Int>()
    var hits = 0

    /** 藏东西的是扫描器，不是钩子，所以这些开关能边跑边翻。 */
...
/** 所有 [ViewRule] 背后就这一个扫描器：每 200 毫秒最多走一遍每个窗口的树。 */
object Views {
    private val watches = Decor<Watch>()

    private fun rules() = Core.switches.filterIsInstance<ViewRule>()
