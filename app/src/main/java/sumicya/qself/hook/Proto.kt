...
/** Just enough protobuf wire format to recognise a recall push. */
object Proto {
    private class Reader(val b: ByteArray, var p: Int = 0, val end: Int = b.size) {
        ...
        /** field, wire type, payload start, payload end — of the next field. */
        fun next(): IntArray {
            val key = varint()
            ...
            when (wire) {
                0 -> { start = p; varint() }
                1 -> { start = p; p += 8 }
                2 -> { val length = varint().toInt(); start = p; p += length }
                5 -> { start = p; p += 4 }
                else -> error("wire type $wire")
            }
    ...
    fun isRecallPush(push: ByteArray): Boolean = runCatching {
        ...
        when {
            ...
            type == 732L && subType == 17L -> {
                // Group notices share 732/17; the recall one carries op_type 7 after a 7-byte prefix.
    ...
    /** [data] without top-level [field]; the same instance when nothing was removed. */
    fun without(data: ByteArray, field: Int): ByteArray = runCatching {
