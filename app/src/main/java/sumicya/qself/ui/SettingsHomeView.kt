...
class SettingsHomeView(context: Context) : LinearLayout(context) {
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (bounded) {
            ...
            // A breakpoint change must not rebuild children during a measure pass: that
            // re-enters layout, which shows up as a frozen screen after repeated taps or
            // a rotation. Post it once and coalesce further requests.
            boundState?.let { state ->
                if (boundCompact != compact() && !rebindPosted) {
                    rebindPosted = true
                    post {
                        rebindPosted = false
    ...
    fun bind(state: State, mode: Int, action: (String) -> Unit) {
        ...
        // ---- section cards: one real second level, no group layer in between ----
        ...
        addView(card {
            navRow("功能开关与错误记录",
                ...
                else "本地功能记录 · 探针日志 · 可复制清空", HomeCatalog.DIAGNOSTICS)
