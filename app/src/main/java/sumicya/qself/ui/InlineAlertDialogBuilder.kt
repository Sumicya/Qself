...
class InlineAlertDialogBuilder @JvmOverloads constructor(context: Context, theme: Int = 0) : AlertDialog.Builder(io.github.qauxv.ui.CommonContextWrapper.createAppCompatContext(context), theme) {
    ...
    private class InlineDialog(context: Context) : AlertDialog(context) {
        ...
        override fun show() {
            ...
            // The host can detach between the availability check and this call (rapid taps,
            // rebinding rows, activity teardown). Dropping the open request is correct here:
            // crashing the host settings screen for a decorative panel is not.
            val handle = InlineSettings.show(context, panel, Runnable {
                if (shown) { shown = false; close = null; dismissListener?.onDismiss(this) }
            }, cancelable, Runnable { cancel() }) ?: return
            close = handle
            shown = true
