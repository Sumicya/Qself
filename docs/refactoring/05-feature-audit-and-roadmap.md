...
- **批次 2（2026-09-07，大重构加速）**: 新增 `表情与贴纸` 子组（auxiliary-emoticon-and-sticker，
  位于聊天和消息之后），迁入 6 项核心表情功能: Telegram 贴纸集/屏蔽超级表情（接收）/移除表情回应限制/
  自定义表情面板列数/表情分享菜单存图/输入Emoji不转大表情。裁定记录: FavEmoticonSortBy 留守收藏与工具
  （批次 1 明文归位，不翻案）；隐藏表情表态留守净化设置（跨顶层树迁移不值）；允许群名带表情语义属群名
  不属表情系统，留守实验。§3 之「隐藏VIP标识子组」裁定撤回: 跨顶层树搬 2 项 standalone 不成组
  （第三处为 SimplifyQQSettingMe 内嵌选项非独立项），现状各得其所。
