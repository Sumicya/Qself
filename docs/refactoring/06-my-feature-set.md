...
| **群管理菜单** | FunBox 复刻（规格已由用户提供） | 🔨 **规格定稿**: 入口=群聊长按头像（顶替原@行为；查共同群可另行分配到点击类交互）。五项: 标记(本地) / 修改群名片 / 撤回消息 / 禁言和踢出 / 查询共同群。实现分级: v1a=✅已落（入口+菜单壳+标记+查询共同群）；v1b=✅**已落（诊断优先）**: GroupAdminBridge 经 AppRuntimeHelper→IKernelService 取 Group/Msg 服务，四动作（名片/撤回/禁言[含时长选择与解除]/移出[确认弹窗]）按关键词+参数形态匹配反射分发，未匹配时 toast+记录 qself_diag.log；菜单首开自动转储候选方法库存（dumpInventoryOnce），**装机一轮采到真名后把匹配钉死为精确签名**（v1b.1 精修项）。仓内群管理 API 存量=零(勘察 2026-09-07)，唯一直接可复用=CheckCommonGroup。FunBox 考古闭环: TG 流传包即精简包(9.59MB，native 已剥)，规格唯一样本=用户记忆 |
...
| 屏蔽更新 ×2 | AntiUpdate(树内，注释即言"instead HookUpgrade") / HookUpgrade(qhmk 第三方) | ✅ 真合并：上游写了替代未删旧；shiply 升级策略路径(upgrade.a.a.c)折入 AntiUpdate 后删除 qhmk 副本，净 -1 |
| 打开资料卡 ×2 | OpenProfileCard.java(共享库+设置入口，5 处调用) / Menu.kt(加号菜单入口) | 改判保留：非重复，共享库双入口模式 |
