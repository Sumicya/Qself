...
**简化 · 现代化 · 自由化**，交互形态可部分参考 Telegram；另含方向词"**原生化**"（语义待确认: 原生 Android 设计语言化？）。
...
- 进场特效/轻互动屏蔽（v1 按迁移记录收入，是否随"通知屏蔽"一起出集？）
- "原生化"的具体含义
- 灰字捕获本体（调试工具，随群日志功能化一并处理？）
...
1. D 节三个存疑点确认 → v2 定稿锁定为壳需求基线；
2. 壳选址（倾向仓内 `:client` 起步，CI/热重载复用）；
3. UI 设计语言由用户出规范，先做设置页原型；
4. 期间本 fork 继续日常主力维护；A 类功能的 9.2.10 验证随日常使用推进。
